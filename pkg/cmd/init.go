...
// NewInitCMD returns "kubeadm init" command.
...
func NewInitCMD(out io.Writer, conf *api.XflopsConfiguration) *cobra.Command {
	initOptions := newInitOptions(conf)
...
func newInitOptions(conf *api.XflopsConfiguration) *initOptions {
	...
	externalClusterCfg := &kubeadmapiv1.ClusterConfiguration{
		...
		ImageRepository:   conf.Repository,
		Networking: kubeadmapiv1.Networking{
			PodSubnet:     conf.PodCIDR,
			ServiceSubnet: conf.ServiceCIDR,
		},
	}
